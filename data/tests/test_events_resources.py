...
class TestStringEvents:
    def test_string_named_listener_fires_for_string_event(self, dispatcher):
        # String-registered listeners were stored but never matched at dispatch.
        hits = []
        dispatcher.listen("user.created", lambda e: hits.append(e))
        dispatcher.dispatch("user.created")
        assert hits == ["user.created"]

    def test_string_listener_matches_event_name_attribute(self, dispatcher):
        hits = []
        ...
        dispatcher.listen("user.created", lambda e: hits.append(e))
        dispatcher.dispatch(Named())
        assert len(hits) == 1

    def test_wildcard_still_hears_string_events(self, dispatcher):
        hits = []
        dispatcher.listen("*", lambda e: hits.append(e))
        dispatcher.dispatch("user.created")
        assert hits == ["user.created"]

    def test_has_listeners_sees_string_registrations(self, dispatcher):
        dispatcher.listen("user.created", lambda e: None)
        assert dispatcher.has_listeners("user.created") is True
        assert dispatcher.has_listeners("other.event") is False
