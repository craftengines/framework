...
<div class="max-w-2xl bg-white border border-slate-200/80 rounded-3xl p-8 shadow-sm">
    ...
    <form action="/posts/{{ post.get_attribute('id') }}" method="POST" class="space-y-6">
        ...
        <div class="space-y-2">
            ...
            <input type="text" name="title" id="title" value="{{ old('title') or post.get_attribute('title') }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500" required>
        </div>

        <div class="space-y-2">
            <label for="body" class="block text-sm font-semibold text-slate-700">Content Body</label>
            <textarea name="body" id="body" rows="6" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-orange-500 focus:ring-1 focus:ring-orange-500" required>{{ old('body') or post.get_attribute('body') }}</textarea>
        </div>

        <div class="flex items-center">
            <input type="checkbox" name="published" id="published" value="1" {% if old('published') or post.get_attribute('published') %}checked{% endif %} class="h-4 w-4 text-orange-600 focus:ring-orange-500 border-slate-300 rounded">
