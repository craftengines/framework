...
Rules
-----
LANG-A  Non-ASCII letters in identifiers, comments or docstrings. Typographic
        punctuation and symbols are language-neutral and allowed; set
        `ascii_only = true` to forbid every non-ASCII character instead.
...
class Config:
    ...
    def load(cls, explicit: Path | None = None) -> "Config":
        ...
        config.ascii_only = bool(raw.get("ascii_only", config.ascii_only))
...
def _has_foreign_letters(text: str, config: Config) -> bool:
    """True when `text` carries non-ASCII *letters* — the mark of foreign prose.

    Typographic punctuation and symbols are language-neutral: an em dash, an
    ellipsis, an arrow, a box-drawing rule or an emoji says nothing about the
    language a comment is written in, and flagging them turns LANG-A into noise
    that engineers learn to ignore. Only letters outside ASCII indicate text in
    another script. Projects that genuinely require an ASCII-only source tree
    set `ascii_only = true` and get the strict behaviour back.
    """
    for char in text:
        if ord(char) <= 127:
            continue
        if config.ascii_only or unicodedata.category(char).startswith("L"):
            return True
    return False
...
def _check_python_tokens(path: Path, source: str, config: Config) -> list[Violation]:
    ...
    for token in tokens:
        if token.type == tokenize.COMMENT:
            if _has_foreign_letters(token.string, config):
                violations.append(
                    Violation(path, token.start[0], token.start[1] + 1, "LANG-A",
                              "non-ASCII letters in a comment — comments are English")
                )
                continue
        ...
        elif token.type == tokenize.NAME and _has_foreign_letters(token.string, config):
            violations.append(
                Violation(path, token.start[0], token.start[1] + 1, "LANG-A",
...
def _check_python_docstrings(tree: ast.AST, path: Path, config: Config) -> list[Violation]:
    ...
    for node in ast.walk(tree):
        ...
        if _has_foreign_letters(docstring, config):
            violations.append(
                Violation(path, line, 1, "LANG-A",
                          "non-ASCII letters in a docstring — docstrings are technical English")
            )
            continue
...
def analyze_generic(path: Path, config: Config, is_template: bool = False) -> list[Violation]:
    ...
    for number, line in enumerate(lines, 1):
        ...
        elif _has_foreign_letters(skeleton, config):
            violations.append(
                Violation(path, number, 1, "LANG-A",
                          "non-ASCII letters outside a string literal")
            )
...
ascii_only = false          # true: forbid every non-ASCII char, not only letters
