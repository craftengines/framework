...
In templates, `@csrf` renders the hidden field. On verification, the token is
accepted only from the request body (`_token`) or the `X-CSRF-TOKEN` header —
never from the query string, where a crafted cross-site link could plant it.
...
Under the `file` driver, saving a session whose id changed — via `regenerate()`
or `invalidate()` — also **destroys the old session file**, so a fixated cookie
cannot keep resurrecting the pre-rotation session server-side.
