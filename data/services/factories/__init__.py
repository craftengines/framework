"""
Factory — Base class for `database/factories/*`; builds attribute dicts via
`definition()`, with `state()`, `count()`/`times()`, `make()` and `create()`.
Category: Core Framework (Factories).
Relations:
  - `create()` persists through the target `Model.create()`
    (`services/orm/model.py`); used in seeders and tests.
References:
  - Guide: `documentation/migrations.md#factories`
"""
# Craft Framework
