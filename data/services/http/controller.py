"""
Controller — Base class for HTTP controllers: view rendering, JSON/redirect
response helpers, and authorization shortcuts.
Category: Core Framework (HTTP).
Relations:
  - Subclassed by `app/Http/Controllers/*`; actions are dispatched by
    `services/http/router.py` and receive a `services/http/request.py` Request.
References:
