...
Relations:
  - Created by `Model.has_many`/`has_one`/`belongs_to`/`belongs_to_many`
    (`services/orm/model.py`); wraps `services/orm/query_builder.py`.
References:
  - Guide: `documentation/orm.md#relationships`
"""
# Craft Framework
...
class BelongsToMany(Relation):
    ...
    def match(self, models: List[Any], results: Any) -> None:
        ...
        grouped: Dict[Any, List[Any]] = {}
        for related in results:
            key = related.get_attribute(self.PIVOT_KEY_ALIAS)
            # Drop the helper alias: it is not a real column, so leaking it
            # into the attributes would pollute to_dict() and break save().
            related._attributes.pop(self.PIVOT_KEY_ALIAS, None)
            grouped.setdefault(key, []).append(related)
    ...
    def attach(self, related_id: Any) -> None:
        ...
        from services.orm.query_builder import _assert_identifier
