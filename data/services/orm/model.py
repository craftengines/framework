...
Relations:
  - Subclassed by `app/Models/*`; builds queries through
    `services/orm/query_builder.py` and relations through
    `services/orm/relationships.py`.
  - Resolves the `db` binding via `Container.getInstance()`
    (`services/container/application.py`).
References:
...
from services.orm.query_builder import QueryBuilder, _MISSING, _assert_identifier
