...
Relations:
  - Bound as `view`, exposed via the `View` facade; rendered by
    `Controller.view()` (`services/http/controller.py`). Errors propagate as
    `TemplateNotFound`/`UndefinedError` — the standalone `view()` helper in
    `services/support` still swallows them (legacy behaviour).
References:
...
def _render_yield(args: str) -> Optional[str]:
    ...
    if match is None:
        return None
    name, value = match.group(1), match.group(2)
    body = value.strip() if value is not None else ""
    return "{% block " + name + " %}" + body + "{% endblock %}"
